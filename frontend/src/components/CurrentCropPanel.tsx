...
export function CurrentCropPanel() {
  ...
  useEffect(() => {
    ...
    reload()
    // Farm Setup is editable without leaving the dashboard. Listen for the
    // save event so the crop card immediately reflects the new crop/date.
    window.addEventListener('farm-updated', reload)
    window.addEventListener('focus', reload)
    return () => {
      alive = false
      window.removeEventListener('farm-updated', reload)
  ...
  const pct = das != null && duration
    ...
    : null
  const harvestDate = crop.expected_harvest_date?.value
  const harvestDays = harvestDate
    ? Math.max(0, Math.ceil((new Date(`${harvestDate}T23:59:59`).getTime() - Date.now()) / 86400000))
    : null
  const daysLeft = harvestDays != null
    ? harvestDays
    : (das != null && duration ? Math.max(0, duration - das) : null)
