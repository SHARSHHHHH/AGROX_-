...
import {
  ...
  getDistrictFunding, getDistrictFundingDetail, getDistrictFundingAvailableStates, getCoverageGap, getFinancialAlerts,
  getDistrictRanking, getWhatChanged, getCropHealthHeatmap, getSatelliteSummary,
  getFarmersBenefited, getMarketplaceActivity, runPestOutbreakScenario,
} from '../services/api'
...
type Tab = 'overview' | 'state' | 'alerts' | 'actions' | 'funding' | 'statefunding'
         | 'districtfunding' | 'schemes' | 'ranking' | 'crophealth' | 'simulator'
         | 'disasterreports'
  ...
  return (
    <div className="max-w-6xl">
      <div className="mb-5 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-field-800">
            🌾 Agriculture Administration Dashboard {currentTabDef && `— ${currentTabDef.icon} ${currentTabDef.label}`}
          </h1>
          <p className="text-sm text-gray-500">
            {currentTabDef?.blurb || 'A clear view of farmer support, district needs and government programmes'} —
            verified government data where available, with derived figures clearly explained.
          </p>
        </div>
      ...
      {tab === 'schemes' && <SchemesTab publish={publish}
        ...
        goDistrictFunding={(_: number) => setParams((p) => { p.set('tab', 'districtfunding'); p.delete('scheme_id'); p.set('fstate', 'Madhya Pradesh'); return p })} />}
...
function DistrictFundingTab({ publish, params, setParams }: any) {
  ...
  const selectedStateFromUrl = params.get('fstate') || 'Madhya Pradesh'
  const [states, setStates] = useState<any[] | null>(null)
  const [state, setState] = useState(selectedStateFromUrl)
  ...
  useEffect(() => {
    getDistrictFundingAvailableStates().then((rows) => setStates(rows)).catch(() => setStates([]))
  }, [])

  useEffect(() => {
    setDistricts(null)
    setDetail(null)
    getDistrictFunding(state, undefined, undefined, sortBy)
      .then(setDistricts).catch(() => setDistricts([]))
    setParams((p: URLSearchParams) => { p.set('fstate', state); p.delete('district'); return p })
  }, [state, sortBy]) // eslint-disable-line

  useEffect(() => {
    if (selectedDistrict) {
      setDetail(null)
      getDistrictFundingDetail(selectedDistrict, state).then(setDetail).catch(() => setDetail(null))
    } else {
      setDetail(null)
    }
  ...
  useEffect(() => {
    ...
    const meta = states.find((s: any) => s.state === state)
    publish('District Funding tab', `${districts.length} ${state} districts with verified PM-KISAN district-level data (${meta?.installment || ''} instalment).`)
  }, [districts, states, state]) // eslint-disable-line

  const openDistrict = (d: string) => setParams((p: URLSearchParams) => { p.set('district', d); p.set('fstate', state); return p })
  const meta = states?.find((s: any) => s.state === state)
  const visible = (districts || []).filter((d: any) => d.district.toLowerCase().includes(query.toLowerCase()))
  const totalBeneficiaries = (districts || []).reduce((sum: number, d: any) => sum + Number(d.beneficiaries || 0), 0)
  const totalAmountCr = Number(meta?.published_total_cr ?? (districts || []).reduce((sum: number, d: any) => sum + Number(d.amount_cr || 0), 0))
  const hasBeneficiaries = (districts || []).some((d: any) => d.beneficiaries != null)

  return (
    <>
      <div className="rounded-2xl bg-gradient-to-r from-field-800 to-field-700 text-white p-5 mb-5 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <div className="text-xs uppercase tracking-wider text-field-100 mb-1">Verified district support · PM-KISAN</div>
            <h2 className="text-xl sm:text-2xl font-bold">District funding & farmer support</h2>
            <p className="text-sm text-field-50/90 mt-1 max-w-3xl">
              Official district-level PM-KISAN figures published by the Government of India. This page does not invent district budget allocations; each state's source, instalment and date are shown below.
            </p>
          </div>
          <div className="flex flex-col gap-2 min-w-[270px]">
            <label className="text-[11px] text-field-100">State</label>
            <select value={state} onChange={(e) => setState(e.target.value)}
              className="rounded-xl bg-white text-field-900 px-3 py-2.5 text-sm font-semibold outline-none">
      ...
      <Card className="mb-5">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3 mb-4">
          <div>
            <h3 className="font-semibold text-field-800">{state}: {districts?.length ?? '—'} verified districts</h3>
            <p className="text-xs text-gray-500 mt-1">{meta?.metric || 'Verified district-level PM-KISAN data'} · {meta?.installment || ''} instalment · {meta?.date || ''}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search district..."
              className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-field-200" />
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
              className="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white">
              <option value="beneficiaries">Most beneficiaries</option>
              <option value="beneficiaries_asc">Fewest beneficiaries</option>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {hasBeneficiaries && <div className="rounded-xl bg-field-50 border border-field-100 px-3 py-2">
            <div className="text-[11px] text-gray-500">Official beneficiaries</div>
            <div className="font-bold text-field-800">{totalBeneficiaries.toLocaleString('en-IN')}</div>
          </div>}
          <div className="rounded-xl bg-gray-50 border border-gray-100 px-3 py-2">
            <div className="text-[11px] text-gray-500">District amount</div>
            <div className="font-bold text-field-800">₹{totalAmountCr.toLocaleString('en-IN', {maximumFractionDigits: 2})} Cr</div>
          </div>
          <div className="rounded-xl bg-gray-50 border border-gray-100 px-3 py-2">
            <div className="text-[11px] text-gray-500">Verification</div>
            <div className="font-bold text-field-800">✓ Government of India</div>
          </div>
        </div>

        {districts === null ? <Spinner /> : visible.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 border-b bg-gray-50/70">
                  ...
                  {hasBeneficiaries && <th className="py-2.5">Beneficiaries</th>}
                  <th className="py-2.5">Amount (₹ Cr)</th>
                  <th className="py-2.5">Status</th>
                </tr>
              </thead>
              <tbody>
                {visible.map((d: any, i: number) => (
                  <tr key={d.district} className={`border-b last:border-0 hover:bg-field-50/60 cursor-pointer transition ${selectedDistrict === d.district ? 'bg-field-50' : ''}`}
                    onClick={() => openDistrict(d.district)}>
                    <td className="py-2.5 px-2 text-gray-400">{i + 1}</td>
                    <td className="py-2.5 font-semibold text-field-800">{d.district}</td>
                    {hasBeneficiaries && <td className="py-2.5 font-medium">{d.beneficiaries == null ? '—' : Number(d.beneficiaries).toLocaleString('en-IN')}</td>}
                    <td className="py-2.5">₹{Number(d.amount_cr || 0).toLocaleString('en-IN', {maximumFractionDigits: 2})}</td>
                    <td className="py-2.5"><DataSourceBadge status="VERIFIED" source={d.source} sourceUrl={d.source_url} lastUpdated={d.last_updated} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        ) : <Empty msg="No verified district dataset is available for this selection." />}
        <div className="mt-3 rounded-lg bg-amber-50 border border-amber-100 p-3 text-[11px] text-amber-900">
          <b>Important:</b> these are verified PM-KISAN district-level support/disbursement figures, not a claim that the state's entire agriculture budget is allocated to each district. The amount type and source date are preserved from the Government of India document.
        </div>
        {meta?.source_url && <a href={meta.source_url} target="_blank" rel="noreferrer" className="inline-block text-[11px] text-field-700 underline mt-2">Open official source document →</a>}
      </Card>
          ...
          {!detail ? <Spinner /> : (
            <div>
              <div className="flex items-start justify-between gap-3 mb-4">
                <div>
                  <div className="text-xs uppercase tracking-wide text-gray-400">Verified district profile</div>
                  <h3 className="text-xl font-bold text-field-800 mt-0.5">📍 {selectedDistrict}, {state}</h3>
                </div>
                <button onClick={() => setParams((p: URLSearchParams) => { p.delete('district'); return p })}
                  className="text-xs border rounded-lg px-3 py-1.5 hover:bg-gray-50">Close</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                {detail.funding?.beneficiaries != null && <StatCard label="PM-KISAN beneficiaries" icon="🧑‍🌾" value={Number(detail.funding.beneficiaries).toLocaleString('en-IN')} />}
                <StatCard label={detail.funding?.amount_type === 'derived_support' ? `${detail.funding?.installment} instalment support*` : `${detail.funding?.installment} instalment amount disbursed`} icon="💰" value={`₹${Number(detail.funding?.amount_cr || 0).toLocaleString('en-IN', {maximumFractionDigits: 2})} Cr`} />
              </div>
              <div className="rounded-xl bg-field-50 border border-field-100 p-3 text-sm text-field-900">
                <b>Verified source:</b> {detail.funding?.source}. Data date: {detail.funding?.data_date}.
              </div>
              <p className="text-[11px] text-gray-400 mt-3">For PM-KISAN, this is scheme support/disbursement at district level; it should not be described as the district's total agriculture budget.</p>
            </div>
          )}
        </Card>
      )}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2 pt-1">
                        ...
                        <button onClick={() => goDistrictFunding(s.scheme_id)}
                          className="text-xs bg-white border rounded-full px-3 py-1 hover:bg-field-50">🏘️ View MP District Support →</button>
                        {detail.url && (
