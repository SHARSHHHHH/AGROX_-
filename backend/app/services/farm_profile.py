...
def _current_crop(farm: Optional[Farm]) -> Dict[str, Any]:
    ...
    # Use calendar dates rather than UTC datetimes so an Indian farmer saving
    # a date today never gets a false "not available" because of a timezone
    # boundary.
    today = datetime.utcnow().date()
    das = None
    days_until_sowing = None
    if farm.sowing_date:
        sowing_day = farm.sowing_date.date()
        delta_days = (today - sowing_day).days
        if delta_days >= 0:
            das = delta_days
        else:
            days_until_sowing = abs(delta_days)

    # If the farmer entered a harvest date but skipped the sowing date, we
    # can still provide a useful countdown. We do NOT pretend the inferred
    # sowing date was entered by the farmer; it is only used to estimate
    # elapsed days when the crop duration is known.
    inferred_sowing_day = None
    if das is None and farm.expected_harvest_date and duration:
        inferred = farm.expected_harvest_date.date() - timedelta(days=duration)
        if inferred <= today:
            inferred_sowing_day = inferred
            das = max(0, (today - inferred).days)
    ...
    return {
        ...
        "sowing_date": _field(
            farm.sowing_date.date().isoformat() if farm.sowing_date else
            (inferred_sowing_day.isoformat() if inferred_sowing_day else None),
            CONFIRMED if farm.sowing_date else (ESTIMATED if inferred_sowing_day else UNAVAILABLE),
            "Estimated backwards from your harvest date and this crop's typical duration."
            if inferred_sowing_day else ""),
        "days_after_sowing": _field(
            das,
            CONFIRMED if farm.sowing_date and das is not None else
            (ESTIMATED if inferred_sowing_day else UNAVAILABLE),
            "Estimated from your harvest date and this crop's typical duration."
            if inferred_sowing_day and not farm.sowing_date else ""),
