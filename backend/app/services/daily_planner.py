...
async def generate_daily_plan(db: Session, user: User,
    ...
    system = (
        "You are a farm planner. Write today's plan as EXACTLY 5 concise numbered "
        "points for the farmer. Return exactly five lines and nothing else. "
        "Each line MUST start with its number and a period: 1., 2., 3., 4., 5. "
        "Cover the most important actions across the WHOLE day: irrigation/watering, "
        "weather warnings, soil or nutrient notes, crop/plant health checks, market "
        "or evening logging when relevant. Use only facts present in the supplied "
        "tasks; do NOT invent data. If there are fewer than five distinct issues, "
        "use useful monitoring/follow-up points from the supplied tasks rather than "
        "making up facts. Write your ENTIRE reply in "
        f"{lang_name}"
